...
# import data -------------------------------------------------------------
alg.rls<-read.csv("kelp/ep_m3_isq.csv") #import seaweed data
...
# Filter data based on specific location and month criteria
...
# quad.spp<-ddply(finaldata_lo,c("survey_id","location","site_code","site_name","quadrat","survey_year","taxon"),summarise, survey.mean = sum(percentage)/5)
...
# neat data into csv ------------------------------------------------------
write.table(quad.site, "kelp/er_atrc_id_v2.csv", sep = ",", row.names = FALSE, col.names = TRUE)
